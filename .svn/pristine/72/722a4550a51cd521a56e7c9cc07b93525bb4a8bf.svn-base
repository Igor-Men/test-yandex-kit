...
$j(document).on('keydown','#id-descriptionshort',function(){
    console.log($j(this).val().length);
    if($j(this).val().length>255){
        $j('#id-descriptionshort').css({'background' : '#FF668C'})
    }else{
        $j('#id-descriptionshort').css({'background' : '#fff'})
    }
})
