    ...
    public function process() {
        try {
            ...
            if ($this->getArgumentSecure('searchin') == 'order') {
                ...
                $users->addWhereQuery('(`id` IN (SELECT `userid` FROM `shoporder` WHERE parentid>0))');
            } elseif ($this->getArgumentSecure('searchin') == 'financepayment_client' && Shop_ModuleLoader::Get()->isImported('finance')) {
                $users->addWhereQuery('(`id` IN (SELECT `clientid` FROM `financepayment`))');
            } elseif ($this->getArgumentSecure('searchin') == 'financepayment_manager' && Shop_ModuleLoader::Get()->isImported('finance')) {
                $searchCompany = false;
