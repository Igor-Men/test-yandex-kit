<?php
class products_view extends Engine_Class {
    ...
    public function process() {
        try {
            ...
            $views = new XShopProductView();
            $views->setProductid($product->getId());
            ...
            if($datebiff >= 60){
                ...
                for($i=$datebiff; $i >= 0; $i--){
                    $d = DateTime_Object::FromString(DateTime_Object::Now()->addMonth(-$i)->__toString())->setFormat('Y-m')->__toString();
                    $b[$d] = array(
                        'date' => $d,
                        'cnt' => 0
                    );
                ...
                for($i=24; $i >= 0; $i--){
                    $d = DateTime_Object::FromString(DateTime_Object::Now()->addHour(-$i)->__toString())->setFormat('Y-m-d H')->__toString();
                    $b[$d] = array(
                        'date' => $d,
                        'cnt' => 0
                    );
                ...
                for($i=$datebiff; $i >= 0; $i--){
                    $d = DateTime_Object::FromString(DateTime_Object::Now()->addDay(-$i)->__toString())->setFormat('Y-m-d')->__toString();
                    $b[$d] = array(
                        'date' => $d,
                        'cnt' => 0
                    );
